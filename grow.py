...
for task in range(10):
    ...
    for ds in ["metal","pallet","std"]:
        ...
        names=[f"embeddings/{ds}{task}_{part}_0.npz" for part in parts]
        ...
        dic={"task":90,"dataset":ds+str(task),"repeat":0,"acc":calc_accuracy(temp,ti,gemb,gi),"checkup":checkup(emb),"collapse":collapse_metrics(emb)}
        
        
        
        np.savez_compressed(f"embeddings/{ds}{task}_90_0.npz",emb=emb,temp=temp,gemb=gemb,i=i,ti=ti,gi=gi,dic=json.dumps(dic,indent=2))
        
        with open(f"results/{ds}{task}_90_0.json","w") as f:
